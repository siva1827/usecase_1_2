...
public class ItemProcessor  {
    ...
    public void validateAndUpdateItem(Exchange exchange) throws Exception {
        ...
        if (!(existingSoldOutObj instanceof Integer)) {
            throw new InventoryValidationException("soldOut must be an integer for item: " + itemId + ", found: " + existingSoldOutObj.getClass().getSimpleName());
        }
        if (!(existingDamagedObj instanceof Integer)) {
            throw new InventoryValidationException("damaged must be an integer for item: " + itemId + ", found: " + existingDamagedObj.getClass().getSimpleName());
        }
        ...
        // Validate lastUpdateDate
        Object lastUpdateDateObj = item.get("lastUpdateDate");
        if (lastUpdateDateObj != null) {
            try {
                LocalDateTime.parse(lastUpdateDateObj.toString(), DATE_TIME_FORMATTER);
                logger.debug("Validated lastUpdateDate for item {}: {}", itemId, lastUpdateDateObj);
            } catch (DateTimeParseException e) {
                throw new InventoryValidationException("Invalid lastUpdateDate format for item: " + itemId + ", expected 'yyyy-MM-dd HH:mm:ss', found: " + lastUpdateDateObj);
            }
        }
        ...
        if (soldOut + damaged > availableStock) {
            throw new InventoryValidationException("Requested quantity exceeds available stock for item ID: " + itemId);
        }
        ...
        item.put("lastUpdateDate", currentDateTime);

        exchange.setProperty("updatedItem", item);
        exchange.getIn().setBody(item);
        logger.debug("Updated item: {}, lastUpdateDate: {}", itemId, item.get("lastUpdateDate"));
    }
